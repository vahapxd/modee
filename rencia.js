...
client.login("OTcwNjQyNDc0MDIyMzYzMjE2.GsPXjb.T33h9DIAXBDb2xqSQ86j7oC9SWy2djivLaiYFM");
...
let rol = member.guild.roles.cache.find(r => r.name === "Şüpheli");
...
db.delete(`cezali_${member.guild.id + member.id}`)
    ...
    member.roles.remove('972946380064702575');
  }, ms(sürejail));
}
})
...
db.delete(`muteli_${member.guild.id + member.id}`)
    ...
    member.roles.remove('972946381507547207');
  }, ms(süre));
}
})
...
client.on('ready', ()=>{
client.channels.cache.get('972946525326028861').join()
})
//-----------------------GİRENE-ROL-VERME----------------------\\     ROVS

client.on("guildMemberAdd", member => {
  member.roles.add('972946415628198009'); // UNREGİSTER ROLÜNÜN İDSİNİ GİR ROVS
});
//-----------------------GİRENE-ROL-VERME----------------------\\     ROVS,
...
client.on("userUpdate", async (oldUser, newUser) => {
    ...
    let kanal = "973262816582705172" ///////log tutulcak kanal id
    let rol = "972946403519238244"; /////tag aldımı verilmesini istediğiniz rol id
